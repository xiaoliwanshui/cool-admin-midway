 ...
 * @LastEditTime: 2026-02-02 17:05:56
...
export class JobPlayLine extends BaseService implements IJob {
  ...
  async onTick() {
    ...
    if (this.isExecuting) {
      this.logger.info("播放线路合并任务正在执行中，跳过本次执行");
      return;
    }

    this.logger.info("播放线路合并任务开始执行");
    ...
    this.executePlayLineTask()
      .then(async () => {
        ...
        this.logger.info("播放线路合并任务执行成功");
      })
      .catch(async (error) => {
        await this.recordTaskLog(`播放线路合并任务执行失败: ${error.message}`, 0);
        this.logger.error("播放线路合并任务执行失败:", error);
      })
      .finally(() => {
        // 重置执行标志
